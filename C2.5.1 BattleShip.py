...
class Board(object):
    ...
    # когда корабль уничтожен необходимо пометить все клетки клетки корабля - уничтожеными
...
if __name__ == '__main__':
    # Делаем список из двух игроков 
    players = []
    players.append(Player(name=username, is_ai=False, auto_ship=True, skill=1))
    players.append(Player(name='PC', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            # в зависимости от результата накидываем сообщений 
            if shot_result == 'miss':
                ...
                game.next_player.message.append('Противник потопил наш корабль :(')
                continue
